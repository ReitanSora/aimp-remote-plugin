 ...
 */
class CGetTrackLyricsTask : public IAIMPTask
{
private:
    /**
    ...
    std::string m_outLyricsUtf8;

public:
    /**
     * @brief Constructs the task.
     * @param plugin Pointer to the plugin used to query playlists. Must not be null.
     * @param playlistId ID of the playlist for which to retrieve track URIs.
     * @param songIndex Index of the song for which to retrieve the URI.
     */
    CGetTrackLyricsTask(MyPlugin* plugin, const std::string& playlistId, int songIndex);
    ...
    std::string GetLyrics() const { return m_outLyricsUtf8; }
